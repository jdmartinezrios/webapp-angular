...
export class UserService {
    ...
    findByRole(rolename: string, req?: any): Observable<HttpResponse<User[]>> {
        const options = createRequestOption(req);
        return this.http.get<User[]>(`${this.resourceUrl}/findByRole/${rolename}` , { params: options, observe: 'response' });
    }

    findByEmail(email: string) {
        return this.http.get<User[]>(`${this.resourceUrl}/findByEmail/${email}` , { observe: 'response' });
    }
