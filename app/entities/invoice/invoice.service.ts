...
export class InvoiceService {
    ...
    findInvoicesToSend(req?: any): Observable<HttpResponse<Invoice[]>> {
        const options = createRequestOption(req);
        return this.http.get<Invoice[]>(this.resourceUrl + '/findInvoicesToSend', { params: options, observe: 'response' })
            .map((res: HttpResponse<Invoice[]>) => this.convertArrayResponse(res));
    }
    ...
    private convertItemFromServer(invoice: Invoice): Invoice {
        ...
        copy.invoiceDate = this.dateUtils
            .convertDateTimeFromServer(invoice.invoiceDate);
    ...
    private convert(invoice: Invoice): Invoice {
        ...
        copy.invoiceDate = this.dateUtils.toDate(invoice.invoiceDate);
