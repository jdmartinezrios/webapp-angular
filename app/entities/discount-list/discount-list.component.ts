...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.discountListService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<DiscountList[]>) => this.onSuccess(res.body, res.headers),
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
        }
        this.discountListService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<DiscountList[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/discount-list'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: DiscountList) {
        return item.id;
    }
    registerChangeInDiscountLists() {
        this.eventSubscriber = this.eventManager.subscribe('discountListListModification', (response) => this.loadAll());
    }
