...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.stockInputService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<StockInput[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/stock-input'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: StockInput) {
        return item.id;
    }
    registerChangeInStockInputs() {
        this.eventSubscriber = this.eventManager.subscribe('stockInputListModification', (response) => this.loadAll());
    }
